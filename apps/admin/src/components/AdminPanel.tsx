...
export type Tab = "operacao" | "usuarios" | "cursos" | "professores" | "matriculas" | "questoes" | "simulados" | "pagamentos" | "cupons" | "configuracoes" | "moderacao" | "auditoria";
...
export function AdminPanel({ initialTab = "operacao" }: { initialTab?: Tab }) {
  ...
  useEffect(() => {
    function syncFromLocation() {
      ...
      const found = tabs.find((item) => item.id === slug);
      setTab(found ? found.id : "operacao");
    }
  ...
  async function selectTab(nextTab: Tab) {
    setTab(nextTab);
    // Deep-linking: mantem a URL sincronizada sem remontar o painel.
    if (typeof window !== "undefined") {
      window.history.pushState({}, "", nextTab === "operacao" ? "/" : `/${nextTab}`);
    }
    setPage(1);
